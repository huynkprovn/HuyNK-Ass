...
namespace KarthusRForRAWR
{
    internal class Program
    {
        ...
        private static void UseSpells(bool useQ, bool useW, bool useE, bool useR, string Source)
        {
            ...
            //W
            if (useW  && W.IsReady() && Player.Distance3D(target) <= W.Range && shouldW(target) &&
            ...
            //E
            if (useE && E.IsReady() && Player.Distance3D(eTar) < E.Range && eSpell.ToggleState == 1 && hasManaForE(Source))
            {
                E.Cast(packets());
            }
                ...
                var target in
                    ObjectManager.Get<Obj_AI_Hero>()
                        .Where(
                            x => !x.IsDead && x.IsEnemy && x.IsVisible))
            {
                if (Q.GetPrediction(target).Hitchance == HitChance.Immobile && qImmo && Player.Distance3D(target) < Q.Range)
                {
                    Q.Cast(target, packets());
                    return;
                }

                if (Q.GetPrediction(target).Hitchance == HitChance.Dashing && qDashing && Player.Distance3D(target) < Q.Range)
                {
                    Q.Cast(target, packets());
                }
            }
        ...
        {
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x =>Player.Distance3D(x) < W.Range && x.IsValidTarget(W.Range) && !x.IsDead && x.IsEnemy &&x.IsVisible))
            {
                if (enemy != null)
                {
        ...
        public static void smartKS()
        {
            ...
            foreach (Obj_AI_Hero target in ObjectManager.Get<Obj_AI_Hero>().Where(x => Player.Distance3D(x) < Q.Range && x.IsValidTarget() && x.IsEnemy && !x.IsDead))
            {
                //Q
        ...
        public static void checkEState()
        {
            ...
            var target = ObjectManager.Get<Obj_AI_Hero>().Count(x => Player.Distance3D(x) < E.Range && x.IsValidTarget(E.Range) && !x.IsDead && x.IsEnemy &&
                                                                     x.IsVisible);
