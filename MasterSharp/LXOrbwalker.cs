...
namespace MasterSharp
{
    public class LxOrbwalker
    {
        ...
        {
            "meditate", "dariusnoxiantacticsonh", "fioraflurry", "garenq",
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (Menu.Item("Move_target").GetValue<bool>() && target != null)
            {
                Orbwalk(
                    target.Path.Count() == 0
                        ? target.Position.To2D().Extend(target.Direction.To2D(), 140).To3D()
                        : target.Position.To2D().Extend(target.Path[0].To2D(), 140).To3D(), target);
            }
            else
            {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                var minionList = MinionManager.GetMinions(MyHero.Position, GetAutoAttackRange() + 500, MinionTypes.All,
        ...
        private static void CheckAutoWindUp()
        {
            ...
            Menu.Item("lxOrbwalker_Misc_ExtraWindUp")
